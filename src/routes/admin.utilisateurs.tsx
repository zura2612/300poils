...
const baseUrl = process.env.ADMIN_WORKER_URL;
...
function AdminUsersPage() {
  ...
  useEffect(() => {
    async function fetchUsers() {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(`admin.utilisateurs.tsx: Erreur serveur: ${response.status}`);
        }
        ...
        console.error("admin.utilisateurs.tsx: Erreur lors de la récupération des utilisateurs:", err);
        setFetchError("admin.utilisateurs.tsx: Impossible de charger la liste des utilisateurs.");
      } finally {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(`admin.utilisateurs.tsx: Erreur serveur: ${response.status}`);
  ...
  // 🔴 REMARQUE : Bloqué en amont par `beforeLoad`. En cas de non-droit d'admin applicatif :
  if (!user || !isAdmin) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-2">
