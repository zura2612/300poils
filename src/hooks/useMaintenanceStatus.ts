...
export function useMaintenanceStatus(): UseMaintenanceStatusResult {
  ...
  useEffect(() => {
    ...
    async function fetchStatus() {
      ...
      const workerUrl = process.env.MAINTENANCE_WORKER_URL;
      const siteName = process.env.SITE_NAME;
      
      if (!workerUrl) {
        console.warn("MAINTENANCE_WORKER_URL non configuré, mode normal par défaut");
        if (isMounted) {
      ...
      if (!siteName) {
        console.warn("SITE_NAME non configuré, mode normal par défaut");
        if (isMounted) {
          setStatus({ mode: "normal" });
          setIsLoading(false);
        }
